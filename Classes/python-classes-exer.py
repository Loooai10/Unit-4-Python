...
#         with self.lock:
#             if not self.closed and amount > 0 and amount <= self.balance:
#                 self.balance -= amount
#             else:
#                 raise ValueError("Account is closed or withdrawl is negative")
#     def close(self):
#         with self.lock:
#             if not self.closed:
#                 self.closed = True
...
class BankAccount:
    def __init__(self ,account_type,current_balance = 0):
    ...
    def deposit(self,amount):
            ...
            return "Deposited -> ",  self.current_balance

    def withdraw(self,amount):
        if self.current_balance >= amount:
            self.current_balance -= amount
            return "Withdrewed -> ",  amount
        else:
...
checking = BankAccount('checkings') #checkings would print invalid account type
...
# print(checking.deposit(transfer)) Gives an error self.current_balance += amount
# TypeError: unsupported operand type(s) for +=: 'int' and 'str'

print("Checking acoount balance is ->", checking.current_balance)
